...
import type { TransportBooking, TransportRequest, TransportQuote, CarrierProfile } from '@/features/transport/types';
...
import { MOCK_BOOKINGS, MOCK_TRANSPORT_REQUESTS, MOCK_QUOTES, MOCK_CARRIER_PROFILES } from '@/lib/mock-data';
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        const request = MOCK_TRANSPORT_REQUESTS.find((r) => r.id === found.requestId);
        const quote = MOCK_QUOTES.find((q) => q.id === found.quoteId);
        const carrier = MOCK_CARRIER_PROFILES.find((c) => c.id === quote?.carrierId);
        setBooking({ ...found, request, quote, carrier });
      } catch {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center" dir="rtl">
        <div className="flex flex-col items-center gap-4 text-center px-4">
          ...
          <Link href="/my-requests" className="btn-primary">
            <ArrowRight size={16} />
            طلباتي
          </Link>
        </div>
