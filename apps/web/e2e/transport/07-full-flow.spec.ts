...
test.describe('Full Flow — Shipper Creates Request', () => {
  test('Shipper creates new request via wizard (A8: single submit)', async ({ page }) => {
    ...
    await expect(goodsBtn).toBeVisible({ timeout: 15000 });
    ...
    await expect(fromGov).toBeVisible({ timeout: 15000 });
    ...
    await expect(toAddr).toBeVisible({ timeout: 15000 });
    ...
    await expect(cargoField).toBeVisible({ timeout: 15000 });
    ...
    await expect(asapBtn).toBeVisible({ timeout: 15000 });
    await asapBtn.click();
    await nextBtn.click();
    await page.waitForLoadState('networkidle');
