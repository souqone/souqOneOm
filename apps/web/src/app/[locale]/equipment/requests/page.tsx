...
import { useEquipmentRequests } from '@/lib/api/equipment';
import type { EquipmentRequestItem } from '@/lib/api/equipment';
...
function RequestCard({ req, locale, tt }: { req: EquipmentRequestItem; locale: string; tt: ReturnType<typeof useTranslations> }) {
  const locationLabel = req.governorate ? resolveLocationLabel(req.governorate, locale) ?? req.governorate : null;
  const budgetText = req.budgetMax
    ? `حتى ${Number(req.budgetMax).toLocaleString('en-US')} ${req.currency}`
    : req.budgetMin
    ? `من ${Number(req.budgetMin).toLocaleString('en-US')} ${req.currency}`
    : null;
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start gap-3 mb-3">
        <div className="w-10 h-10 rounded-xl bg-blue-50 dark:bg-blue-900/20 flex items-center justify-center shrink-0 group-hover:scale-105 transition-transform">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[11px] text-on-surface-variant mt-0.5">
            {EQUIP_TYPE_LABEL[req.equipmentType] ?? req.equipmentType}
            {req.quantity > 1 && ` · الكمية: ${req.quantity}`}
          </p>
        </div>
        <span className={`shrink-0 text-white text-[10px] font-bold px-2 py-0.5 rounded-lg ${STATUS_COLORS[req.requestStatus] ?? 'bg-gray-500'}`}>
          {STATUS_LABELS[req.requestStatus] ?? req.requestStatus}
        </span>
      </div>

      {/* Description */}
      <p className="text-[12px] text-on-surface-variant line-clamp-2 mb-3 flex-1">{req.description}</p>
      ...
      <div className="flex flex-wrap gap-1.5 mb-3">
        ...
        {req.startDate && (
          <span className="text-[10px] bg-surface-container text-on-surface-variant px-2 py-0.5 rounded-full font-medium">
            {new Date(req.startDate).toLocaleDateString('ar-EG', { month: 'short', day: 'numeric' })}
          </span>
        )}
        {req.rentalDuration && (
          <span className="text-[10px] bg-surface-container text-on-surface-variant px-2 py-0.5 rounded-full font-medium">
      ...
      <div className="flex items-center justify-between pt-3 border-t border-outline-variant/10">
        <div className="flex items-center gap-3">
          {budgetText && (
            <span className="text-[13px] font-black text-amber-600">{budgetText}</span>
          )}
          {locationLabel && (
      ...
      <p className="text-[10px] text-on-surface-variant/50 mt-2">
        {relativeTimeT(req.createdAt, tt, locale)}
      </p>
    </Link>
  );
}
...
export default function EquipmentRequestsPage() {
  ...
  const params: Record<string, string> = { page: String(page), limit: '12', requestStatus: 'OPEN' };
  ...
  const { data, isLoading, error } = useEquipmentRequests(params);
  ...
  return (
    <>
      <main className="min-h-screen bg-background">
        ...
        {/* ── Hero ── */}
        <div className="bg-gradient-to-bl from-blue-800 via-blue-700 to-indigo-800 px-4 pt-8 pb-10 relative overflow-hidden">
          ...
          <div className="relative max-w-3xl mx-auto text-center">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/15 backdrop-blur-sm text-white/80 text-[11px] font-medium mb-3">
              <span className="material-symbols-outlined text-[14px] text-blue-300">assignment</span>
              سوق المعدات الثقيلة
            </div>
            <h1 className="text-xl sm:text-2xl md:text-3xl font-black text-white mb-2">
              طلبات المعدات المفتوحة
            </h1>
            <p className="text-white/70 text-sm mb-5">
              تصفح الطلبات المفتوحة وقدّم عرضك مباشرةً لأصحاب الطلبات
            </p>
            {/* Search */}
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-4">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            >
              ...
              {hasActiveFilters && (
                <span className="w-5 h-5 rounded-full bg-white/30 flex items-center justify-center text-[10px] font-black">
                  {[equipmentType, governorate, withOperator].filter(Boolean).length}
                </span>
              )}
            </button>
            ...
            <Link
              ...
              className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold bg-amber-600 text-white hover:bg-amber-700 transition-colors ms-auto"
            >
            ...
            {meta && (
              <span className="text-sm text-on-surface-variant">
                {meta.total.toLocaleString('en-US')} طلب
              </span>
            )}
          </div>
          ...
          {showFilters && (
            <div className="mt-3 p-4 bg-surface-container-lowest rounded-2xl border border-outline-variant/20 grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              <div>
                ...
                <select
                  value={withOperator}
                  onChange={e => { setWithOperator(e.target.value as '' | 'true' | 'false'); setPage(1); }}
                  className="w-full bg-surface-container border border-outline-variant/20 rounded-xl px-3 py-2 text-sm font-medium text-on-surface focus:ring-2 focus:ring-primary/30 focus:border-primary transition-all"
                >
              ...
              {hasActiveFilters && (
                <button
                  ...
                  className="sm:col-span-3 flex items-center gap-1.5 text-xs font-bold text-red-500 hover:text-red-600 transition-colors w-fit"
                >
                  <X size={13} />
                  مسح الفلاتر
                </button>
          ...
          {error ? (
            ...
            <div className="flex flex-col items-center justify-center py-24 text-center">
              <div className="w-16 h-16 rounded-full bg-blue-50 dark:bg-blue-900/20 flex items-center justify-center mb-4">
                <span className="material-symbols-outlined text-blue-500 text-3xl">assignment_late</span>
              </div>
              <h3 className="text-base font-bold text-on-surface mb-2">لا توجد طلبات مفتوحة</h3>
            ...
            <>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {items.map(req => (
                  <RequestCard key={req.id} req={req} locale={locale} tt={tt} />
                ))}
              </div>
