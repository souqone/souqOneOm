 ...
 * Fixed bottom bar with price and action buttons.
 */
    ...
    >
      ...
      <div className="flex items-center gap-2">
        ...
        {hasPhone && onCall && (
          <button
            onClick={onCall}
            className="h-10 px-4 rounded-xl border border-outline-variant/30 text-on-surface text-[13px] font-medium flex items-center justify-center gap-1.5 hover:border-primary hover:text-primary transition-colors"
          >
            <Phone size={15} />
            {ts('call')}
          </button>
