...
export function DriverNavTabs({ active, onChange, counts, variant = 'tabs', className = '' }: DriverNavTabsProps) {
  ...
  if (variant === 'sidebar') {
    return (
      <div className={`bg-surface-container-lowest rounded-2xl border border-outline-variant/15 shadow-sm overflow-hidden ${className}`}>
        {TABS.map((tab, i) => (
          <button
            key={tab.key}
            type="button"
  ...
  return (
    <div className={`sticky top-14 z-10 bg-background border-b border-outline-variant/[0.08] -mt-4 rounded-t-2xl ${className}`}>
